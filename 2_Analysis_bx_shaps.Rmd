...
pad_data_hasbrain %>%
  ...
  labs(
    x = "Visit",
    y = "Stress Severity",
    title = "Stress Severity over 2 Years"
  ) # on average, no change in anhedonia sx severity over time (which is expected), but there is a lot of variability within and between persons
```

> doesn't look like any outlying shaps scores. checking sk

```{r}
library(moments)
# distribution of shaps at baseline
pad_and_rois_v0 <-
  pad_data_hasbrain %>%
  filter(visit == 0)
pad_and_rois_v0 %>%
  ...
  geom_histogram() +
  theme_classic()
skewness(pad_and_rois_v0$shaps_c)
kurtosis(pad_and_rois_v0$shaps_c)

# distribution of shaps at fu 1
pad_and_rois_v1 <-
  pad_data_hasbrain %>%
  filter(visit == 1)
pad_and_rois_v1 %>%
  ...
  geom_histogram() +
  theme_classic()
skewness(pad_and_rois_v1$shaps_c, na.rm = TRUE) 
kurtosis(pad_and_rois_v1$shaps_c, na.rm = TRUE) 

# distribution of shaps at fu 3
pad_and_rois_v3 <-
  pad_data_hasbrain %>%
  filter(visit == 3)
pad_and_rois_v3 %>%
  ...
  geom_histogram() +
  theme_classic()
skewness(pad_and_rois_v3$shaps_c, na.rm = TRUE) 
kurtosis(pad_and_rois_v3$shaps_c, na.rm = TRUE) 

# distribution of shaps at fu 6
pad_and_rois_v6 <-
  pad_data_hasbrain %>%
  filter(visit == 6)
pad_and_rois_v6 %>%
  ...
  geom_histogram() +
  theme_classic()
skewness(pad_and_rois_v6$shaps_c, na.rm = TRUE) 
kurtosis(pad_and_rois_v6$shaps_c, na.rm = TRUE) 

# distribution of shaps at fu 9
pad_and_rois_v9 <-
  pad_data_hasbrain %>%
  filter(visit == 9)
pad_and_rois_v9 %>%
  ...
  geom_histogram() +
  theme_classic()
skewness(pad_and_rois_v9$shaps_c, na.rm = TRUE) 
kurtosis(pad_and_rois_v9$shaps_c, na.rm = TRUE) 

# distribution of shaps at fu 12
pad_and_rois_v12 <-
  pad_data_hasbrain %>%
  filter(visit == 12)
pad_and_rois_v12 %>%
  ...
  geom_histogram() +
  theme_classic()
skewness(pad_and_rois_v12$shaps_c, na.rm = TRUE) 
kurtosis(pad_and_rois_v12$shaps_c, na.rm = TRUE) 

# distribution of shaps at fu 15
pad_and_rois_v15 <-
  pad_data_hasbrain %>%
  filter(visit == 15)
pad_and_rois_v15 %>%
  ...
  geom_histogram() +
  theme_classic()
skewness(pad_and_rois_v15$shaps_c, na.rm = TRUE)
kurtosis(pad_and_rois_v15$shaps_c, na.rm = TRUE)  

# distribution of shaps at fu 18
pad_and_rois_v18 <-
  pad_data_hasbrain %>%
  filter(visit == 18)
pad_and_rois_v18 %>%
  ...
  geom_histogram() +
  theme_classic()
skewness(pad_and_rois_v18$shaps_c, na.rm = TRUE) 
kurtosis(pad_and_rois_v18$shaps_c, na.rm = TRUE) 

# distribution of shaps at fu 21
pad_and_rois_v21 <-
  pad_data_hasbrain %>%
  filter(visit == 21)
pad_and_rois_v21 %>%
  ...
  geom_histogram() +
  theme_classic()
skewness(pad_and_rois_v21$shaps_c, na.rm = TRUE) 
kurtosis(pad_and_rois_v21$shaps_c, na.rm = TRUE) 
...
pad_and_rois_v24 %>%
  ...
  geom_histogram() +
  theme_classic()
skewness(pad_and_rois_v24$shaps_c, na.rm = TRUE) 
kurtosis(pad_and_rois_v24$shaps_c, na.rm = TRUE) 
```
...
pad_and_rois_outliers <-
  pad_data_hasbrain %>%
  ungroup() %>%
  identify_outliers(shaps_c) %>%
  filter(is.extreme == TRUE) # 0 extreme
```
...
means_by_visit <-
  ...
  dplyr::summarise(
    mean_shaps = round(mean(shaps_c), 2),
    sd_shaps = round(sd(shaps_c), 2)
  )
write_csv(means_by_visit, "shaps_means_by_visit.csv")
```

# Plotting overall distributions of anhedonia and stress
...
summary(pad_data_hasbrain$aleq_c)
sd(pad_data_hasbrain$aleq_c, na.rm = TRUE)
```
...
```{r}
library(forecast)
Acf(visit.means3_L1andL2vars$shaps_c, type = "correlation", plot = TRUE, na.action = na.contiguous)
...
summary(mlm_ri) # Group, Sex, Age, between and within-person stress
...
summary(mlm_rs) # Group, Sex, Age, between and within-person stress
...
```{r}
sjPlot::tab_model(mlm_rs, bootstrap = TRUE, iterations = 1000, dv.labels = c("Anhedonia Severity"), seed = 1234, pred.labels = c("Intercept", "Between-person Stress", "Within-person Stress", "Visit", "Age", "Sex [F]", "Tanner", "Risk Group [High]"), file = "Anhedonia_Covariates.doc")
```
...
write_csv(pad_and_rois_usable, "pad_and_rois_usable_shaps.csv")
```
